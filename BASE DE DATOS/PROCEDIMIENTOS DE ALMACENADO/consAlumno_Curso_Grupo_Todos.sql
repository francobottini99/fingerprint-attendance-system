CREATE DEFINER=`root`@`localhost` PROCEDURE `consAlumno_Curso_Grupo_Todos`(
	in aIdCurso int(11),
    in aIdGrupo int(11),
    in aIdCl int(11)
)
BEGIN
	...
	WHERE
		Grupo.Curso_idCurso = aIdCurso AND Alumnos.Grupo_idGrupo = aIdGrupo AND Alumnos.CicloLectivo_idCiclo_Lectivo = aIdCl;
END